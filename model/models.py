...
class WordRNN(nn.Module):
    ...
    def forward(self, topic, caps, caplens, new_batch_size, states):
        ...
        #print(topic.shape)
        #print(densecap_captions.shape)
                
        # try torch.mm
        #merged = torch.mul(topic, densecap_captions)
        #merged = torch.mean(merged, 1, False)
        #print(merged.shape)
        #merged = merged.unsqueeze(1)
        
        #self.decode_step.flatten_parameters()
        ...
        predictions = torch.zeros(self.batch_size, max(caplens), self.vocab_size).to(device)
        ...
        return outputs, caplens, caps, sort_ind, h, c

...
\textit{image paragraph generation} has been initially introduced in \cite{krause2016hierarchical}, proposing the challenge of generating a text, consisting of several sentences that would form a coherent whole.
...
Attention has also been employed in formal theories of interaction. One of the approaches has been proposed by \newcite{Dobnik2016AMF}, who link attention with judgements as defined in Type Theory of Records \cite{Cooper08typetheory}. % SD 2020-06-09 10:48:27 +0200: A more relevant description here would be thay we provide a Bayesian model of bottom-up and top-down attention in judgements classification of Type Theory... \cite{Cooperinprepa}
...
% SD 2020-06-09 10:38:27 +0200: Since our approach is based on Krause we should start with other methods first and then lead to Krause and also give an overview of the main components (e.g. graph).
...
% models: baseline and experiments, training data, etc.
% evaluation: attention maps on humans and on generated captions (attended image features (?))
...
% SD 2020-06-09 10:44:41 +0200: We should explain Krause model better to give an intuition what is happening between different components. Or rather, we should give a description (including a diagram) of our model and then also mention in what ways it is different from Krause. Otherwise, it is hard to follow the following description.

...
To achieve sufficient coherence in generated texts, several important NLG-related tasks are typically considered: \textit{content determination (selection)}, the task of deciding which parts of the source information should be included in the output description, and \textit{text structuring}, the task of ordering selected information \cite{Gatt2017}.
We believe that the nature of these tasks is reflected in the hierarchical nature of our models: first, the model attends to the image objects and defines both their salience and order of mention, and then it starts to linguistically realise them.
...
Along with the n-gram penalty, which we leave for the future work, we believe that setting $C$ would provide us with more varied and diverse sentences.
Similar to \newcite{opennmt2017}, we ensure that each generated sentences consists of at least $C$ words by setting \verb|p(<end>) = -1e20| if it has been chosen by the beam before the sentence minimal length $C$ is achieved.
...
To measure diversity, we report vocabulary size of our models and number of noun types generated in paragraphs to approximate how many unique objects are mentioned.
...
We calculate self-BLEU as follows: we split each generated paragraph into sentences and use one sentence as hypothesis and the other are regarded as references.
Higher score indicates less diversity, e.g. more \textit{n}-gram matches between compared sentences.

% word mover distance metric (?)
...
Also, the models which do not control for parameter $C$ perform much worse than the models which are forced to generate a minimum number of words in a sentence.
...
However, it is unclear whether such behaviour has a positive effect on diversity of paragraphs: $C\sp{+}$ models might simply produce repetitive \textit{n}-grams.
...
And indeed, as Table~\ref{tab:divs} demonstrates, $C\sp{-}$ models generally produce more diverse output in terms of vocabulary items than the $C\sp{+}$ models.

...
\centering
 ...
 \caption{
 Example image with generated paragraphs from three different models.\\
...
We demonstrate that multimodal input paired with attention on these modalities benefits model's ability to generate more diverse and accurate paragraphs. % SD 2020-08-28 12:54:31 +0200: also more accurate? +
...
Our model is different from the model by \newcite{krause2016hierarchical} in the following ways: (i) we use either max-pooling or attention in our models, (ii) we do not learn to predict the end of the paragraph, but generate the same number of sentences as we find in ground-truth paragraph per each image.
% SD 2020-08-30 22:50:59 +0200: Krause also does not use semantic information.
The focus of our work is not to improve on the results of \newcite{krause2016hierarchical} but to investigate the effects of different multi-modal fusion on the accuracy of diversity of paragraph descriptions. % SD 2020-08-28 14:22:15 +0200: Added this sentence to emphasise our relation to Krause.
...
We describe six configurations of our model, which we train, validate and test on the released paragraph dataset splits (14,575, 2,487, 2,489 for training, validation and testing respectively) \cite{krause2016hierarchical}. % SD 2020-08-29 11:10:48 +0200: Citation for the dataset.
...
As shown by human evaluation, looking at the overall mean, the multi-modal information does help generation of better paragraphs when using max-pooling.
...
The performance of the \textbf{IMG+LNG} model with max polling is close to the performance of the \textbf{IMG} model while the performance of the \textbf{LNG} model is slightly lower.
Overall, attention is judged as more advantageous in general than max pooling, having higher mean scores over all criteria compared to the mean scores of max-pooling models.
...
Interestingly, max-pooling does not seem to have the same effect on utilisation of semantic information: the \textbf{LNG} model achieves the lowest scores. A possible explanation for this is that with max-pooling the same semantic information is chosen for every sentence topic while attention learns to select different semantic information for a sequence of topics. This appears to affect semantic features more than visual features.
...
Overall, the results indicate that both visual and semantic information are beneficial for the generated paragraphs as they effect different evaluation categories differently. The main challenge lines in information fusion of visual and semantic information in the model with attention. We believe that these results suggest the following future experiments: (i) detailed investigation of early vs. late attention (when to fuse two modalities and how), (ii) as \newcite{vanmiltenburg2017} argue, more control over human evaluation can provide us with better, more precise human judgements, (iii) training with other decoding strategies such as top-\textit{k} sampling or nucleus sampling \cite{Holtzman2019}.
...
%Also, we note that the result of this work is a series of future experiments 
